...
# 899050 北证50 bj899050
# 000300 沪深300 sh000300

#* DONE index = 1 大盘指数的历史指数的获取
def market_history_fetch(symbol:str = "000300", period="daily" ,start_date:str = "20220101", end_date:str = "20240531",say:int = 0) -> tuple[bool,str]:
    ...
    try:
        ...
        stock_zh_a_hist_df = ak.index_zh_a_hist(symbol=symbol, period=period, start_date=start_date, end_date=end_date)
        # stock_zh_a_hist_df = ak.stock_zh_index_daily(symbol=symbol)
    ...
    df.to_json('market_history.json',orient='records', force_ascii=False, indent=4)
    return True,f"Successfully"

# a = market_history_fetch()
# print(a)

#* DONE index = 2 大盘指数的历史指数的处理和存入数据库
def market_history_save(say:int = 0)->tuple[bool, str]:
    ...
    market_history_core(mycursor, symbol='000001', flag='sh', name='上证指数', period="daily", start_date='20220101', end_date=today, say=0)
    market_history_core(mycursor, symbol='399001', flag='sz', name='深证成指', period="daily", start_date='20220101', end_date=today, say=0)
    market_history_core(mycursor, symbol='899050', flag='bj', name='北证50', period="daily", start_date='20220101', end_date=today, say=0)
    market_history_core(mycursor, symbol='000300', flag='sh', name='沪深300', period="daily", start_date='20220101', end_date=today, say=0)

    user_db.commit()

    return True,f'Successfully'

def market_history_core(mycursor, symbol:'000001', flag:str = 'sh',name:str = '上证指数', period="daily", start_date='20220101', end_date='20240531',say:int = 0) ->bool:
    '将指数历史数据存入数据库的核心操作'
    zh_symbol = flag + symbol

    tf,mess = market_history_fetch(symbol=symbol,period=period, start_date=start_date, end_date=end_date)
    if tf == False:
        if say:
            print(f"-->{mess}")
    if tf == True:
        history_file = open('market_history.json','r',encoding='utf-8')
        ...
        # break
        sql2 = f'''INSERT INTO {zh_symbol} (time, market_code, market_name, exchange, open_price, close_price,
