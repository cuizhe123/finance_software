...
class BacktestAnalyzer:
    ...
    def prepare_data(self):
        ...
        for file in tqdm(factor_files):
            ...
            if  file in ret_files and date_tmp >= self.start_date and date_tmp <= self.end_date:
                ...
                # final = res_fr.replace([np.inf, -np.inf], 0)
    ...
    def calculate_effectiveness(self):
        for factor in self.ic:
            ...
            ic = round(np.mean(self.ic[factor]), 3)
            icir = round((np.mean(self.ic[factor]) / np.std(self.ic[factor])), 3)
            long_ret = round(np.mean(self.pnl[factor][1]), 3)
            long_std = round((np.std(self.pnl[factor][1]) * np.sqrt(252)), 3)
            long_ir = round((np.mean(self.pnl[factor][1]) * 252 / (np.std(self.pnl[factor][1]) * np.sqrt(252))),3)
            long_drawdown = round(((cumulative_returns[1].cummax() - cumulative_returns[1]) / cumulative_returns[1].cummax()),3)
            long_maxdd = long_drawdown[long_drawdown.argmax()] * 100
            

            hedge_ret = round((np.mean(self.pnl[factor]['hedge']) * 252),3)
            hedge_std = round((np.std(self.pnl[factor]['hedge']) * np.sqrt(252)),3)
            hedge_ir = round((np.mean(self.pnl[factor]['hedge']) * 252 / (np.std(self.pnl[factor]['hedge']) * np.sqrt(252))), 3)
            hedge_drawdown =round(((cumulative_returns['hedge'].cummax() - cumulative_returns['hedge']) / cumulative_returns['hedge'].cummax()), 3)
...
if __name__ == '__main__':
    ...
    end_date = '2022-04-01'
    trade_date = './fundmental/date.pkl'
    factor_path = './feature/xgb_pred'
    ret_path = './fundmental/data_ret'
    ud_path = './fundmental/data_ud'
